...
class TestAsyncApi(unittest.TestCase):
    ...
    def test_redirect_to_root(self):
        ...
        for method, expected in featured_methods.items():
            ...
            assert res.content_type == expected[1], '{}: {}'.format(method, expected)
            if expected[0] == 'GET':
                assert res.location == expected[2], '{}: {}'.format(method, expected)

    def test_configure(self):
        route = '/api/v{}/configure'.format(version)
        featured_methods = {
            'GET': [200, 'application/json'],
            # Should return a 400 if validation has errors, which this POST will return
            # since the ssh_user is an integer not a string.
            'POST': [
                400,
                'application/json',
                '{"ssh_config": {"ssh_user": 1}}'],
            'PUT': [405, 'text/plain'],
            'DELETE': [405, 'text/plain'],
            'HEAD': [405, 'text/plain'],
            'TRACE': [405, 'text/plain'],
    ...
    def test_configure_status(self):
